...
장비: SM-S901N (S22, One UI 8.0 / Android 16) · 무선 adb · `adb forward tcp:3100 tcp:3000`

| # | 항목 | 결과 | 근거 |
|---|---|---|---|
| 1 | 대시보드 **화면 잠금 → 요청 0** | ✅ | SSE 40초 관찰: `tick 1회 + beat 2회` (이전엔 tick 40회). tick 1회당 클라이언트가 4요청이므로 **160요청 → 4요청** |
| 2 | `visibilitychange` 시 연결·타이머 정리 | ✅ | 배포 JS 블록을 Node 에서 실제 실행해 20/20 통과 (`scripts/verify_dashboard_realtime.js`) |
| 3 | SSE 재연결 지수 백오프 | ✅ | 관측값 `1.2s → 2.1s → 4.2s → 8.2s → 16.0s` (고정 5초 아님, 상한 60초) |
| 4 | 루트 rename 차단 | ✅ | `{"error":"이름 없음 또는 루트 경로 금지"}` + 루트 디렉터리 보존 확인 |
| 5 | MCP 경로 탈출 차단 | ✅ | `경로 탈출 차단` / 정상 경로 정상 응답 |
| 6 | StatusPages 안전망 | ✅ | `500 {"error":"server_error","detail":"JSONException"}` (이전엔 응답 본문 자체가 없었음) |
| 7 | 속도 제한 단독 토글 | ✅ | dl=5MB/s 설정 시 **ul=3MB/s 유지** (이전엔 0으로 덮어써짐) |
| 8 | 가드 임계치 즉시 반영 | ✅ | thermalLimit=52 즉시 반영 (이전 최대 5분 지연) |
| 9 | 속도 값 상한 | ✅ | `Long.MAX_VALUE` → 10GiB/s, `maxActive=999` → 10 |
| 10 | `peers.json` 유휴 시 쓰기 | ✅ | 70초 관찰 중 **0회** (이전 분당 12회) |
| 11 | 토렌트 활성 한도 즉시 적용 | ✅ | maxActive 2→5 변경 → `토렌트 활성 한도 적용 maxActive=5` 로그 |
| 12 | 업로드 메모리 안정 | ✅ | 40MB 업로드 시 PSS **1.928GB → 1.940GB (+11MB)**. 버퍼링이었다면 +40MB, base64 였다면 +130MB. 로그에 `스트리밍` 표시 |
| 13 | 토렌트 영속화 가드 | ✅ | 저장 11회 중 10회가 실제 상태 전이 6초 이내에 대응 |

**커버리지**: 13/13 통과. 테스트 213 → **222건**.

### 발견 — Ktor 기본 multipart 50MB 제한

600MB 업로드가 아래와 같이 실패했다.

```
Limit of 52428800 bytes exceeded while searching for "----DLWiuG9..."
```

`ktor-server-core` 의 기본 multipart 파트 제한(50MB)이며 **이번 수정으로 추가된 제약이 아니다**
(기존에도 동일하게 실패했다). 다만 오류가 사용자에게 그대로 노출되고,
대용량 업로드 경로가 `/api/storage/raw-upload`(스트리밍, 제한 없음)뿐이라는 점이 드러났다.

→ **후속 과제**: `multipart` 파트의 한계를 올리거나, 대용량은 `raw-upload` 로 유도하는
클라이언트 분기를 명시할 것.
...
| 항목 | 사유 |
|---|---|
| 웹 인증 스위치 거부 동작 | API 엔드포인트가 없고 **앱 UI 전용**. UI 조작 없이 확인 불가. 위험 상태(auth=true + 암호 없음)는 현재 존재하지 않음을 로그로 확인(`auth=false`) |
| 토렌트 pause 연타 → UI 멈춤 없음 | **사용자 토렌트 3건이 실제 다운로드 중**(`prog=0.64`, 2.8GB). 일시정지시키지 않음. 대신 코드상 `withContext(Dispatchers.IO)` 적용 확인 |
